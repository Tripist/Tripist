...
public class RecyclerViewSquaresMapAdapterTest {


    public ActivityTestRule<Squares_Category> mActivitiyTestRule = new ActivityTestRule<>(Squares_Category.class);
    ...
    public void recyclerScrollandItemTestReligions(){
        ...
        RecyclerView recyclerView = mActivitiyTestRule.getActivity().findViewById(R.id.square_rv);
        int itemcount = recyclerView.getAdapter().getItemCount();
        Espresso.onView(withId(R.id.square_rv)).perform(RecyclerViewActions.scrollToPosition(itemcount-1));
        Espresso.onView(withId(R.id.square_rv)).perform(RecyclerViewActions.actionOnItemAtPosition(itemcount-1,click()));
    }
